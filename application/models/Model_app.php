...
class Model_app extends CI_model{
    ...
    public function view_address($table,$order){
        $this->db->select('*, city.name as city, state.name as state, country.name as country');
        $this->db->from($table);
        $this->db->join('city', $table.'.city_id=city.city_id');
    ...
    public function view_join_provinsi($order){
        ...
        $this->db->join('country b', 'a.country_id=b.country_id');
        $this->db->order_by($order,"DESC");
        return $this->db->get()->result_array();
    }
    ...
    public function view_join_kota($order){
        ...
        $this->db->from('city a');
        $this->db->join('state b', 'a.state_id=b.state_id');
        $this->db->join('country c', 'b.country_id=c.country_id');
        $this->db->order_by($order,"DESC");
        return $this->db->get()->result_array();
    }
    ...
    public function stok($id){
        return $this->db->query("SELECT (sum(z.jml)-sum(z.kurang)) as stok FROM 
                                    (SELECT sum(a.jml_terima) as jml, null as kurang FROM pembelian_terima_detail a JOIN pembelian_terima b ON a.id_pembelian_terima=b.id_pembelian_terima where a.id_barang='$id' 
                                    UNION
                                    SELECT null as jml, sum(a.jml_return) as kurang FROM pembelian_return_detail a JOIN pembelian_return b ON a.id_pembelian_return=b.id_pembelian_return where a.id_barang='$id'
                                    UNION 
                                    SELECT sum(tambah) as jml, sum(kurang) as kurang FROM penyesuaian_detail where id_barang='$id'
                                    UNION 
                                    SELECT null as jml, sum(b.jumlah_jual*b.jumlah_satuan) as kurang FROM `transaksi` a JOIN transaksi_detail b ON a.id_transaksi=b.id_transaksi where b.id_barang='$id'
                                    UNION
                                    SELECT sum(a.jumlah_return) as jml, null as kurang FROM `transaksi_return_detail` a JOIN transaksi_detail b ON a.id_transaksi_detail=b.id_transaksi_detail JOIN transaksi_return c ON a.id_transaksi_return=c.id_transaksi_return where c.status='selesai' AND b.id_barang='$id') as z");
    }

    public function view_barang(){
        return $this->db->query("SELECT * FROM barang a JOIN kategori b ON a.id_kategori=b.id_kategori");
    }

    public function view_barang_where($status){
        return $this->db->query("SELECT * FROM barang a JOIN kategori b ON a.id_kategori=b.id_kategori where a.status_jual='$status'");
    }
    ...
    public function view_pembelian(){
        return $this->db->query("SELECT a.*, b.nama_karyawan, c.nama_supplier, d.nama_type_bayar, sum(jml_pesan*harga_pesan) as tbayar FROM pembelian a 
                                    JOIN karyawan b ON a.id_karyawan=b.id_karyawan 
                                        JOIN supplier c ON a.id_supplier=c.id_supplier
                                            JOIN type_bayar d ON a.id_type_bayar=d.id_type_bayar
                                                JOIN pembelian_detail e ON a.id_pembelian=e.id_pembelian GROUP BY a.id_pembelian");
    }
    ...
    public function pembelian_detail($id){
        return $this->db->query("SELECT a.*, b.nama_karyawan, c.nama_supplier, d.nama_type_bayar, sum(jml_pesan*harga_pesan) as tbayar FROM pembelian a 
                                    JOIN karyawan b ON a.id_karyawan=b.id_karyawan 
                                        JOIN supplier c ON a.id_supplier=c.id_supplier
                                            JOIN type_bayar d ON a.id_type_bayar=d.id_type_bayar
                                                JOIN pembelian_detail e ON a.id_pembelian=e.id_pembelian where a.id_pembelian='$id'");
    }
    ...
    public function pembelian_detail_edit($id){
        return $this->db->query("SELECT b.id_karyawan, b.id_pembelian, b.kode_pembelian, b.tgl_kirim, c.nama_karyawan, d.nama_supplier, e.nama_type_bayar, b.keterangan as ket_pembelian, b.tgl_pembelian, b.referensi, b.kepada_attention, 
                                    ...
                                    FROM `pembelian_terima` a 
                                        JOIN pembelian b ON a.id_pembelian=b.id_pembelian 
                                            JOIN karyawan c ON b.id_karyawan=c.id_karyawan 
                                                JOIN supplier d ON b.id_supplier=d.id_supplier
                                                    JOIN type_bayar e ON b.id_type_bayar=e.id_type_bayar
                                                        JOIN karyawan f ON a.id_karyawan=f.id_karyawan 
    ...
    public function edit_pembelian_return($id){
        return $this->db->query("SELECT z.id_pembelian_return, z.no_return, z.tgl_kirim_return, z.id_type_bayar as id_type_bayar_return, z.keterangan_return, z.tanggal_return, b.id_pembelian, b.kode_pembelian, b.tgl_kirim, c.nama_karyawan, d.nama_supplier, e.nama_type_bayar, b.keterangan as ket_pembelian, b.tgl_pembelian, b.referensi, b.kepada_attention, 
                                    a.id_pembelian_terima, a.no_pembelian_terima, a.no_surat_jalan, a.pengirim_barang, a.keterangan as ket_po, a.tanggal_terima, f.nama_karyawan as penerima_barang
                                    FROM pembelian_return z 
                                        JOIN pembelian_terima a ON z.id_pembelian_terima=a.id_pembelian_terima
                                            JOIN pembelian b ON a.id_pembelian=b.id_pembelian 
                                                JOIN karyawan c ON b.id_karyawan=c.id_karyawan 
                                                    JOIN supplier d ON b.id_supplier=d.id_supplier
                                                        JOIN type_bayar e ON b.id_type_bayar=e.id_type_bayar
                                                ...
                                                JOIN supplier d ON b.id_supplier=d.id_supplier
                                                    JOIN type_bayar e ON b.id_type_bayar=e.id_type_bayar
                                                        JOIN karyawan f ON a.id_karyawan=f.id_karyawan 
                                                            ...
                                                            JOIN pembelian_return g ON a.id_pembelian_terima=g.id_pembelian_terima
                                                            JOIN type_bayar h ON g.id_type_bayar=h.id_type_bayar
                                                            where a.id_pembelian_terima='$id'");
    }

    public function view_pembelian_terima(){
        return $this->db->query("SELECT a.*, b.kode_pembelian, b.tgl_pembelian, c.nama_karyawan, d.nama_supplier FROM `pembelian_terima` a 
                                    JOIN pembelian b ON a.id_pembelian=b.id_pembelian 
    ...
    public function view_pembelian_return(){
        return $this->db->query("SELECT a.*, c.kode_pembelian, d.nama_karyawan, e.nama_supplier, f.nama_type_bayar 
                                    FROM `pembelian_return` a 
                                        JOIN pembelian_terima b ON a.id_pembelian_terima=b.id_pembelian_terima 
                                            JOIN pembelian c ON b.id_pembelian=c.id_pembelian 
                                                JOIN karyawan d ON c.id_karyawan=d.id_karyawan 
                                                    JOIN supplier e ON c.id_supplier=e.id_supplier 
                                                        JOIN type_bayar f ON c.id_type_bayar=f.id_type_bayar ");
    }

    public function view_penyesuaian(){
        return $this->db->query("SELECT a.*, b.nama_sebab_alasan FROM penyesuaian a JOIN sebab_alasan b ON a.id_sebab_alasan=b.id_sebab_alasan ORDER BY a.id_penyesuaian DESC");
    }
    ...
    public function view_join_city($table1,$table2,$field,$order){
        ...
        $this->db->from($table1);
        $this->db->join($table2, $table1.'.'.$field.'='.$table2.'.'.$field);
        $this->db->join('city', $table1.'.city_id=city.city_id');
        $this->db->order_by($order,"DESC");
        return $this->db->get()->result_array();
    }

    public function list_transaksi_penjualan($status){
        return $this->db->query("SELECT a.id_transaksi, a.waktu_proses, d.nama_karyawan, b.nama_pelanggan, c.nama_type_bayar, a.jumlah_bayar, a.keterangan FROM `transaksi` a LEFT JOIN pelanggan b ON a.id_pelanggan=b.id_pelanggan LEFT JOIN type_bayar c ON a.id_type_bayar=c.id_type_bayar JOIN karyawan d ON a.id_karyawan=d.id_karyawan where a.status='$status'");
    }

    public function list_transaksi_return($status){
        return $this->db->query("SELECT a.id_transaksi_return, a.waktu_return, d.nama_karyawan, b.nama_pelanggan, c.nama_type_bayar, a.bayar_return, a.ket_return FROM `transaksi_return` a JOIN transaksi e ON a.id_transaksi=e.id_transaksi LEFT JOIN pelanggan b ON e.id_pelanggan=b.id_pelanggan LEFT JOIN type_bayar c ON a.id_type_bayar=c.id_type_bayar JOIN karyawan d ON a.id_karyawan=d.id_karyawan where a.status='$status'");
    }

    public function cek_promosi_barang($id_barang,$id_transaksi){
        $query = $this->db->query("SELECT *, REPLACE(tgl_mulai,'-',''), REPLACE(tgl_selesai,'-','') FROM promosi where kode_terapkan='barang' AND beli_barang='$id_barang' AND '".date('Ymd')."'>=REPLACE(tgl_mulai,'-','') AND '".date('Ymd')."'<=REPLACE(tgl_selesai,'-','')");
        if ($query->num_rows()>=1){
            $promo = $query->row_array();
            if ($promo['jml_beli']==1){
                $stok_gratis = $this->model_app->stok($promo['bonus_barang'])->row_array();
                if ($stok_gratis['stok']>=$promo['jml_bonus']){
                    $trxx = $this->db->query("SELECT * FROM barang_harga where id_kategori_pelanggan='1' AND id_jenis_jual='1' AND id_barang='".$promo['bonus_barang']."'")->row_array();
    ...
    public function cek_promosi_barang_update($id_barang,$id_transaksi,$jml,$id_kategori_pelanggan,$satuan,$id_jenis_jual){
        ...
        if ($query->num_rows()>=1){
            ...
            $bonusx = floor($jml/$promo['jml_beli'])*$promo['jml_bonus'];
                $trxx = $this->db->query("SELECT * FROM barang_harga where id_barang='$promo[bonus_barang]' AND satuan='$satuan'")->row_array();
                $stok_gratis = $this->model_app->stok($promo['bonus_barang'])->row_array();
                if ($stok_gratis['stok']>=$bonusx){
                    $cekkeranjang = $this->db->query("SELECT * FROM transaksi_detail where id_transaksi='$id_transaksi' AND id_barang='$promo[bonus_barang]' AND status='0'");
                        ...
                        $cek_diskon = $this->db->query("SELECT $statusjual as diskon FROM promosi_detail where id_promosi='$promo[id_promosi]' AND id_kategori_pelanggan='$id_kategori_pelanggan'")->row_array();
                        ...
                        $this->db->query("UPDATE transaksi_detail SET diskon_jual=diskon_jual+$diskonrp where id_transaksi='$id_transaksi' AND id_barang='$promo[beli_barang]' AND status='1'");
                    ...
                    if ($jml/$promo['jml_beli']<1){
                        $this->db->query("DELETE FROM transaksi_detail where id_barang='$promo[bonus_barang]' AND id_transaksi='$id_transaksi' AND status='0'");
                    }
                }
        }
    }
