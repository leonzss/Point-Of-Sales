...
<?php 
  ...
  $spp = $this->db->query("SELECT a.kepada_attention, b.*, c.name as city, d.name as state, e.name as country FROM `pembelian` a 
                            JOIN supplier b ON a.id_supplier=b.id_supplier 
                              JOIN city c ON b.city_id=c.city_id
                                JOIN state d ON b.state_id=d.state_id
                                  JOIN country e ON b.country_id=e.country_id
          ...
          echo "<h2><center>PURCHASE RECEIVE</center></h2>
                ...
                <table id='tablemodul1' width='100%' border='1'>
                      ...
                      <tbody>";
                        ...
                        $no = 1;
                        $penyesuaian_detail = $this->db->query("SELECT * FROM pembelian_detail a JOIN barang b ON a.id_barang=b.id_barang where a.id_pembelian='$row[id_pembelian]' ORDER BY a.id_pembelian_detail DESC");
                        foreach ($penyesuaian_detail->result_array() as $r){
                          $stok = $this->model_app->view_one('barang',array('id_barang' => $r['id_barang']),'id_barang')->row_array();
                          $stok_tambah = $this->db->query("SELECT sum(jml_terima) as stok_tambah FROM `pembelian_terima_detail` where id_barang='".$r['id_barang']."'")->row_array();
                          $stok_kurang = $this->db->query("SELECT sum(jml_return) as stok_kurang FROM `pembelian_return_detail` where id_barang='".$r['id_barang']."'")->row_array();
                          $trm = $this->db->query("SELECT sum(a.jml_terima) as terima FROM `pembelian_terima_detail` a JOIN pembelian_terima b ON a.id_pembelian_terima=b.id_pembelian_terima where a.id_pembelian_terima='".$this->uri->segment(3)."' AND a.id_barang='".$r['id_barang']."'")->row_array();
                        ...
                        $tot = $this->db->query("SELECT sum(jml_terima) as jml, sum(harga_pembelian*jml_terima) as total FROM `pembelian_terima_detail` where id_pembelian_terima='".$this->uri->segment(3)."'")->row_array();
                      echo "</tbody>
