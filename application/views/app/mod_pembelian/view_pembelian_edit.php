          ...
          echo "<div class='col-md-6'>
                ...
                <table class='table table-bordered table-striped'>
                      ...
                      <tbody>";
                        ...
                        $no = 1;
                        $penyesuaian_detail = $this->db->query("SELECT * FROM pembelian_detail a JOIN barang b ON a.id_barang=b.id_barang where a.id_pembelian='".$this->uri->segment(3)."' ORDER BY a.id_pembelian_detail DESC");
                        foreach ($penyesuaian_detail->result_array() as $r){
                          $terima = $this->db->query("SELECT sum(a.jml_terima) as jml_terima, b.id_pembelian, a.* FROM pembelian_terima_detail a JOIN pembelian_terima b ON a.id_pembelian_terima=b.id_pembelian_terima where a.id_barang='$r[id_barang]' AND b.id_pembelian='".$this->uri->segment(3)."'")->row_array();
                          echo "<tr>
                                <td>$no</td>
                                <td>$r[nama_barang]</td>
                                <td>$r[jml_pesan]</td>
                        ...
                        $tot = $this->db->query("SELECT sum(jml_pesan*harga_pesan) as tharga, sum(jml_pesan) as tjumlah FROM `pembelian_detail` where id_pembelian='".$this->uri->segment(3)."'")->row_array();
                        $ter = $this->db->query("SELECT sum(jml_terima) as tterima FROM pembelian_terima_detail a JOIN pembelian_terima b ON a.id_pembelian_terima=b.id_pembelian_terima where b.id_pembelian='".$this->uri->segment(3)."'")->row_array();
                      echo "</tbody>
