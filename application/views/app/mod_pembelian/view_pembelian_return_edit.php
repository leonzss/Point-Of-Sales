...
<?php 
    echo "<div class='col-md-12'>
              ...
              echo form_open_multipart('app/edit_pembelian_return',$attributes); 
                $tgltrm = $this->db->query("SELECT GROUP_CONCAT(DATE_FORMAT(tanggal_terima, '%d/%m/%Y') SEPARATOR ', ') as tanggal  FROM `pembelian_terima` where id_pembelian='".$row['id_pembelian']."'")->row_array();
          echo "<div class='col-md-6'>
                ...
                <table class='table table-bordered table-striped'>
                      ...
                      <tbody>";
                        ...
                        $no = 1;
                        $penyesuaian_detail = $this->db->query("SELECT a.*, b.nama_barang, b.harga_beli, b.kode_satuan FROM `pembelian_return_detail` a JOIN barang b ON a.id_barang=b.id_barang where id_pembelian_return='".$this->uri->segment(3)."'");
                        foreach ($penyesuaian_detail->result_array() as $r){
                          $stok = $this->model_app->stok($r['id_barang'])->row_array();
                          $brg = $this->db->query("SELECT * FROM `pembelian_detail` where id_barang='".$r['id_barang']."' AND id_pembelian='$row[id_pembelian]'")->row_array();
