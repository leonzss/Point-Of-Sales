<?php 
  $ppn = $this->db->query("SELECT * FROM conf_penjualan ORDER BY id_conf_penjualan DESC LIMIT 1")->row_array();
          ...
          echo "<div class='col-md-9'>
                 ...
                 echo "</select><input class='hidden btn btn-success btn-sm' type='submit' name='submit'>
                  ...
                  echo "<table style='margin-top:5px' class='table table-bordered table-striped table-condensed'>
                        ...
                        <tbody>";
                          $no = 1;
                          $transaksi_detail = $this->db->query("SELECT a.*, b.nama_barang FROM transaksi_detail a JOIN barang b ON a.id_barang=b.id_barang where a.id_transaksi='".$this->session->id_transaksi."' ORDER BY a.id_transaksi_detail DESC");
                                  ...
                                  if ($r['status']=='1'){
                                    echo "<td><input type='number' name='jml[$no]' value='$r[jumlah_jual]' style='width:100%; text-align:center'></td>
                                          <td><select name='satuan[$no]' value='$r[kode_satuan]' style='width:100%; padding:2px'>";
                                            $satuan = $this->db->query("SELECT * FROM barang_harga where id_barang='$r[id_barang]' AND id_kategori_pelanggan='$id_kategori_pelanggan'");
                    ...
                    $brt = $this->db->query("SELECT sum(jumlah_jual*berat_bruto) as berat FROM `transaksi_detail` a JOIN barang b ON a.id_barang=b.id_barang where id_transaksi='".$this->session->id_transaksi."' AND status='1'")->row_array();
                    $tot = $this->db->query("SELECT sum((jumlah_jual*harga_jual)-diskon_jual) as subtotal FROM `transaksi_detail` where id_transaksi='".$this->session->id_transaksi."'")->row_array();
                    $tot2 = $this->db->query("SELECT (biaya_kirim-diskon_persen/100*biaya_kirim-diskon_rupiah) as total, gratis_kirim, diskon_belanja, jumlah_bayar, keterangan FROM `transaksi` where id_transaksi='".$this->session->id_transaksi."'")->row_array();
