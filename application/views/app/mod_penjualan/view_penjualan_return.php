<?php 
    echo "<div class='col-md-12'>
                  ...
                  echo "<table style='margin-top:5px' class='table table-bordered table-striped table-condensed'>
                        ...
                        <tbody>";
                          ...
                          if ($this->session->id_transaksi_return!=''){
                            $transaksi_detail = $this->db->query("SELECT a.*, b.nama_barang FROM transaksi_detail a JOIN barang b ON a.id_barang=b.id_barang where a.id_transaksi='".$this->session->id_transaksi_return."' ORDER BY a.id_transaksi_detail DESC");
                            foreach ($transaksi_detail->result_array() as $r){
                              $row = $this->db->query("SELECT * FROM transaksi_return_detail where id_transaksi_return='".$this->session->id_return."' AND id_transaksi_detail='$r[id_transaksi_detail]'")->row_array();
          ...
          $tot2 = $this->db->query("SELECT bayar_return, ket_return FROM `transaksi_return` where id_transaksi_return='".$this->session->id_return."'")->row_array();
