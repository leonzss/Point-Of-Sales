...
<?php 
          echo "<table border='0' class='left'>
                ...
                <table width='100%' border='0'>
                      <tbody>";
                        $no = 1;
                        $transaksi_detail = $this->db->query("SELECT a.*, b.nama_barang FROM transaksi_detail a JOIN barang b ON a.id_barang=b.id_barang where a.id_transaksi='".$this->uri->segment(3)."' ORDER BY a.id_transaksi_detail DESC");
                    ...
                    $tot = $this->db->query("SELECT sum((jumlah_jual*harga_jual)-diskon_jual) as subtotal FROM `transaksi_detail` where id_transaksi='".$this->uri->segment(3)."' AND status='1'")->row_array();
                    $tot2 = $this->db->query("SELECT (biaya_kirim-diskon_persen/100*biaya_kirim-diskon_rupiah) as total, gratis_kirim, diskon_belanja, jumlah_bayar, keterangan FROM `transaksi` where id_transaksi='".$this->uri->segment(3)."'")->row_array();
                    ...
                    $item = $this->db->query("SELECT * FROM transaksi_detail where id_transaksi='".$this->uri->segment(3)."'")->num_rows();
                    $hitung_ppn = $this->db->query("SELECT sum(b.ppn/100*(((a.jumlah_jual*a.jumlah_satuan)*a.harga_jual)-a.diskon_jual)) as ppn FROM `transaksi_detail` a JOIN barang b On a.id_barang=b.id_barang where a.id_transaksi='".$this->uri->segment(3)."'")->row_array();
