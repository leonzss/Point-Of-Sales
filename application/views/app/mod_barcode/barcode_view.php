...
<body onload="window.print()">
    <?php
      ...
      $row = $this->model_app->view_one_address('conf_perusahaan',array('id_perusahaan' => 1),'id_perusahaan')->row_array();
			for ($i=1; $i <= $jml_data; $i++){
          for ($b=1; $b <= $this->input->post('b')[$i]; $b++){ 
            $rows = $this->model_app->view_one('barang',array('kode_barang' => $this->input->post('a')[$i]),'id_barang')->row_array();
